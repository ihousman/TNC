from zonal_stats_extractor_lib import *
#//Module imports
...
def addPrefixToCollectionBandNames(c,prefix):
    ...
    def catFun(i):return ee.String(i).cat(prefix)
    outBandNames = bandNames.map(catFun)
    return c.select(bandNames,outBandNames)

def addSuffixToCollectionBandNames(c,suffix):
    bandNames = ee.Image(c.first()).bandNames()
    def catFun(i):return ee.String(i).cat(suffix)
    outBandNames = bandNames.map(catFun)
    return c.select(bandNames,outBandNames)
...
sa = ee.FeatureCollection('projects/igde-work/igde-data/igde_buffer_20k_union_for_clipping').geometry();
...
indexNames = ['NBR','NDMI','NDVI','SAVI','EVI','brightness','greenness','wetness','tcAngleBG']
indexNamesComposites = ['NBR','NDMI','NDVI','SAVI','EVI','brightness','greenness','wetness','tcAngleBG','NIRv']

indexEndWildcards =   ['.*' + i for i in  indexNames]
indexStartWildcards = [i+'.*' for i in indexNames]

#// var igdes = ee.FeatureCollection('projects/igde-work/igde-data/GDEpulse2018_iGDE_V1_20180802_joined_annual_depth_macro_veg');
igdes = ee.FeatureCollection('projects/igde-work/igde-data/iGDE_AnnualDepth_renamed_oct2018_v2');
#print(igdes.size().getInfo())
#// var igdeCount = 15419;//igdes.size().getInfo();
#// var igdesL = igdes.toList(10000000,0);
#
howMany = 500
composites = ee.ImageCollection('projects/igde-work/raster-data/composite-collection')\
        .sort('system:time_start')\
        .map(lambda img: multBands(img,1,0.0001))\
...
lt = ee.ImageCollection('projects/igde-work/raster-data/LANDTRENDR-collection')\
        .select(indexEndWildcards)\
        .map(lambda img:multBands(img,1,0.0001))
#
#print(lt.first().getInfo())
harmonics = ee.ImageCollection('projects/igde-work/raster-data/harmonic-coefficients-collection');

zTrend =ee.ImageCollection('projects/igde-work/raster-data/z-score-trend-collection');

daymet = ee.ImageCollection('projects/igde-work/raster-data/DAYMET-Collection');
daymet = addPrefixToCollectionBandNames(daymet,'dymt_');
z = zTrend.select(['.*_Z']).map(lambda img:multBands(img,1,0.1))
trend = zTrend.select(['.*._slope']).map(lambda img: multBands(img,1,0.0001));
zTrend = joinCollections(z,trend);
zTrend1 = zTrend.filter(ee.Filter.calendarRange(121,185)).map(resetDate)
zTrend2 = zTrend.filter(ee.Filter.calendarRange(185,249)).map(resetDate)
zTrend1 = addSuffixToCollectionBandNames(zTrend1,'_121_185');
zTrend2 = addSuffixToCollectionBandNames(zTrend2,'_185_249');
zTrend = joinCollections(zTrend1,zTrend2)


pap = harmonics\
    .map(lambda img: multBands(img,1,0.0001))\
    .map(getPhaseAmplitudePeak)\
